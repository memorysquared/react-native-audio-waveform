...
export const Waveform = forwardRef((props, ref) => {
    ...
    playbackSpeed = 1.0, isExternalUrl = false, downloadExternalAudio = true, candleSpace = 2, candleWidth = 5, containerStyle = {}, waveColor, scrubColor, onPlayerStateChange, onRecorderStateChange, onPanStateChange = () => { }, onError = () => { }, onCurrentProgressChange = () => { }, candleHeightScale = 3, onChangeWaveformLoadState, onDownloadStateChange, onDownloadProgressChange, } = props;
    const viewRef = useRef(null);
    const [audioPath, setAudioPath] = useState(!isExternalUrl ? path : undefined);
    const scrollRef = useRef(null);
    const isLayoutCalculated = useRef(false);
    const audioPathRef = useRef(undefined);
    ...
    const setExternalAudioPath = (filePath) => {
        setAudioPath(filePath);
        audioPathRef.current = filePath;
        onDownloadStateChange?.(false);
        onDownloadProgressChange?.(100);
    };
    /**
     ...
     */
    const downloadAndCacheFile = async (fileUrl, fileName) => {
        ...
        try {
            ...
            await RNFetchBlob.config({
                ...
                let progressPercentage = Number(((received / total) * 100)?.toFixed?.(2));
                onDownloadProgressChange?.(progressPercentage);
            })
                .then((response) => {
    ...
    const checkIsFileDownloaded = async (fileName) => {
        const filePath = `${cacheDir}/${fileName}`;
        const fileExists = await RNFetchBlob.fs.exists(filePath);
        if (fileExists) {
            setExternalAudioPath(filePath);
    ...
    useEffect(() => {
        const fileName = formatUrlToFileName(path);
    ...
    const preparePlayerForPath = async (progress) => {
        if (!isNil(audioPath) && !isEmpty(audioPath)) {
            ...
            return Promise.reject(new Error(`Can not start player for path: ${audioPath}`));
        }
    };
    const getAudioDuration = async () => {
        try {
            ...
            if (!isNil(duration)) {
                ...
                return Promise.reject(new Error(`Could not get duration for path: ${audioPath}`));
            }
        }
        catch (err) {
            return Promise.reject(err);
    ...
    const getAudioWaveFormForPath = async (noOfSample) => {
        if (!isNil(audioPath) && !isEmpty(audioPath)) {
            ...
            onError(`Can not find waveform for mode ${mode} path: ${audioPath}`);
            console.error(`Can not find waveform for mode ${mode} path: ${audioPath}`);
        }
    };
        ...
        if (mode === 'static') {
            try {
                ...
                if (!isNil(result) && result) {
                    ...
                    return Promise.reject(new Error(`error in stopping player for path: ${audioPath}`));
                }
            }
            catch (err) {
                return Promise.reject(err);
        ...
        if (mode === 'static') {
            try {
                ...
                if (play) {
                    ...
                    return Promise.reject(new Error(`error in starting player for path: ${audioPath}`));
                }
            }
            catch (error) {
        ...
        if (mode === 'static') {
            try {
                ...
                if (pause) {
                    ...
                    return Promise.reject(new Error(`error in pause player for path: ${audioPath}`));
                }
            }
            catch (error) {
                return Promise.reject(error);
    ...
    return (React.createElement(View, { style: [styles.waveformContainer, containerStyle] },
        React.createElement(View, { ref: viewRef, style: styles.waveformInnerContainer, onLayout: calculateLayout, ...(mode === 'static' ? panResponder.panHandlers : {}) },
            React.createElement(ScrollView, { horizontal: true, ref: scrollRef, style: styles.scrollContainer, scrollEnabled: mode === 'live' }, waveform?.map?.((amplitude, indexCandle) => (React.createElement(WaveformCandle, { key: indexCandle, index: indexCandle, amplitude: amplitude, parentViewLayout: viewLayout, candleWidth,
                candleSpace,
                noOfSamples,
                songDuration,
                currentProgress,
